...
# 배열에 저장된 값들
##### 배열에 저장된 값에는 각기 다른 번호가 할당되며, 이 번호를 통해 원하는 값에 접근할 수 있다. 중요한 것은 목록의 첫 번째 값부터 (1아 아닌) 0이 할당된다는 점이다.
##### 배열값에 번호 매기기 
- ##### 배열의 각 아이템에는 자동적으로 인덱스라고 부르는 번호가 매겨진다. 이 번호를 이용하여 배열 내의 특정 아이템에 접근이 가능하다. 아래와 같이 세 개의 색상 이름을 가지고 있는 배열이 있다고 가정해보자
```js
    var colors = new Array('white',
                           'black',
                           'custom');
```
##### 조금 햇갈리지만 인덱스 값은 (1이 아닌) 0부터 시작한다. 다음 표는 인덱스 값과 그에 해당하는 배열 값을 보여준다.
***
# 배열 아이템에 접근하기
- ##### 배열의 세번째 아이템을 가져오려면 배열 이름 뒤에 대괄호를 이용하여 해당 아이템의 인덱스 값을 지정하면 된다. 아래는 itemThree라는 이름의 변수를 선언하고 colors 배열의 세 번째 값을 가져와 할당하는 예제이다.
```js
    var itemThree;
    itemThree = colors[2];
```
***
# 배열에 저장된 아이템의 개수
- ##### 각각의 배열은 length라는 이름의 속성을 가지고 있다. 이 속성은 배열에 저장된 아이템의 개수를 알려준다. 아래 예제는 numColors라는 변수를 선언하고 배열의 아이템 개수를 그 값으로 할당하는 방법을 보여준다.
```js
    var numColors;
...
***
# 배열 내의 값에 접근하여 수정하기
##### 왼쪽 예제의 첫 번째 줄의 코드는 세개의 색상 이름을 저장하는 배열을 생성한다.(배열 값들은 한 줄에 모두 나열해도 되고 예제처럼 여러 줄로 나누어 써도 무방하다). 배열을 생성하고 나면 세 번째 아이템을 'custom'에서 'beige'로 변경한다. 배열 값에 접근하려면 배열 이름 다음에 접근하고자 하는 값의 인덱스 번호를 대괄호를 이용해서 지정하면 된다. 이렇게 선택한 배열의 아이템에 변수를 사용할 때와 마찬가지 방법으로 새로운 값을 대입하면 해당 아이템의 값을 변경할 수 있다.
***
# 표현식(expression)은 하나의 값으로 평가될 수 있는 문장이다. 자바스크립트에는 크게 두 가지 종류의 표현식이 존재한다.
#### 변수에 값을 대입하는 표현식
##### 실제로 변수는 어떤 값을 저장하고 있어야 비로소 쓸모가 있다. 지금까지의 예제에서 살펴보았듯이 변수에 값을 저장할 때는 대입 연산자(등호 기호)를 사용한다.
```js
    var color = 'beige';
    이 표현식의 결과로 변수 color는 beige라는 값을 갖는다.
```
#### 하나의 값을 리턴하기 위해 두 개 혹은 그 이상의 값을 사용하는 표현식
##### 하나의 값을 판단하기 위해 여러 개의 값에 대한 연산을 수행할 수도 있다. 다음 예제를 살펴보자
```js
    var area = 3 * 2;
    이 경우 area 변수 값은 6이 된다.
```
##### 예제에서 3 * 2라는 표현식은 6이라는 값으로 평가된다. 또한 표현식 3*2 의 결과 area라는 이름의 변수에 저장된다. 두 개의 문자열을 결합하여 하나의 문자열로 만드는 것도 두 개의 값을 사용하는 표현식의 또 다른 예라고 할 수 있다.
***
# 표현식은 연산자(operator)에 의존적이다. 연산자는 개발자가 하나 혹은 그 이상의 값을 하나의 값으로 만들 때 사용한다.
```js
    대입 연산자
    변수에 값을 대입한다.
    var color = 'beige';
    이제 color 변수 값은 beige라는 값을 갖는다.
```
***
```js
    산술 연산자
...
```js
    ...
    두 개의 문자열을 결합한다.
    greeting = '안녕하세요,' + '웹 지니';
    이제 greeting 변수 값은 '안녕하세요, 웹지니'이다.
```
